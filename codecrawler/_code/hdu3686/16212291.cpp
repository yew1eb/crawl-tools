...
const int maxn = 10000 + 10;    // ç¹çä¸ªæ°
const int maxm = 100000 + 10;   // è¾¹çä¸ªæ°
...
int E, time, top, btot;
vector<int> belo[maxn]; //ç¹å±äºåªä¸ªå
        ...
        if(!dfn[to]) {
            ...
            if(low[to] >= dfn[u]) {
                ...
                do {
                    ...
                    bian[edge[now].id] = btot;  //ãè®°å½æè¾¹å±äºåªä¸ªå
                    to = edge[now].u;
                }while(to != u);
            }
        }
...
bool treecut[maxn<<1];  // ç¼©ç¹ææ åæ¯ä¸ªç¹æ¯å¦ä¸ºå²ç¹
