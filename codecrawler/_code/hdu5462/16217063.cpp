...
namespace Vectorial {
    /* ç¹ç§¯: ä¸¤åéé¿åº¦çä¹ç§¯åä¹ä¸å®ä»¬å¤¹è§çä½å¼¦, å¤¹è§å¤§äº90åº¦æ¶ç¹ç§¯ä¸ºè´ */
    double getDot (Vector a, Vector b) { return a.x * b.x + a.y * b.y; }

    /* åç§¯: åç§¯ç­äºä¸¤åéç»æçä¸è§å½¢æåé¢ç§¯çä¸¤å, cross(v, w) = -cross(w, v) */
...
namespace Polygonal {
    ...
    int isPointInPolygon (Point o, Point* p, int n) {
        ...
        for (int i = 0; i < n; i++) {
            int j = (i + 1) % n;
            if (onSegment(o, p[i], p[j]) || o == p[i]) return 0; // è¾¹çä¸
    ...
    /* è®¡ç®åå¹³é¢ç¸äº¤å¯ä»¥ç¨å¢éæ³ï¼o(n^2)ï¼åå§è®¾ç½®4æ¡æ ç©·å¤§çåå¹³é¢ */
    /* ç¨æåç´çº¿A->Båå²å¤è¾¹å½¢uï¼è¿åå·¦ä¾§ãå¯è½éåæåç¹æçº¿æ®µ */
...
namespace Circular {
    ...
    /* a[i] å b[i] åå«æ¯ç¬¬iæ¡åçº¿å¨O1åO2ä¸çåç¹ */
    /* have some problems */
...
void solve () {
    for (int i = 1; i <= N; i++) {
        ...
        for (int j = 0; j < 4; j++)
            L[cnt++] = DirLine(P2[j], P2[(j+1)%4]-P2[j]);

        int n = halfPlaneIntersection(L, cnt, P);
        printf(" %d", (int)(getArea(P, n) + 0.5));
    }
    printf("\n");
}
