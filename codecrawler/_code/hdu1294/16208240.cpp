...
LL dp[maxn]={0,1,1,2,4,9,20,48,115,286,719,1842,4766,12486,32973,87811,235381,634847,1721159,4688676,
                ...
                4113254119923150,11703780079612453};//å¯ä»¥ç´æ¥æè¡¨äºãã
LL e[maxn],t,s;
LL C(LL n,LL m)//æ±ç»åæ°
{
    m=min(m,n-m);
    LL i,s=1;
    for(i=1;i<=m;i++)
...
/*
    å¯éå¤éæ©çç»åãænä¸ªåç´ ï¼æ¯ä¸ªåç´ å¯ä»¥éå¤æ¬¡ï¼ä¸å±ékä¸ªè¢æ¯ï¼æå¤å°ç§éæ©ï¼
    n=3,k=2,æ6ç§:(1,1),(1,2),(1,3),(2,2),(2,3),(3,3)
    å± C(k+n-1,n-1)=C(n+k-1,k)ç§

    æ¬é¢ï¼æ±nä¸ªç»ç¹æ°çæ æå¤å°ç§ï¼å¯éç¨æ´æ°åån-1çæ¹æ³
    n=5æ¶ï¼åå4=1+1+1+1=1+1+2=1+3=2+2=4ã
    ådp[5]=C(dp[1]+4-1,4)+C(dp[1]+2-1,2)*dp[2]+dp[1]*dp[3]+dp[2]*dp[2]+dp[4]=9;
