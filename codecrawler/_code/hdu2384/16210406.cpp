...
int cmp( const void *a , const void *b )
{
    ...
    if(c->icount != d->icount)
        return d->icount - c->icount;//æ»é¢æ°éåº
    else if(c->tol_time!=d->tol_time)
        return c->tol_time - d->tol_time;//æ»æ¶é´ååº
    else
    {
        for(int i=c->icount; i>=1; i--)
        {
            if(c->a[i] != d->a[i])
                return c->a[i] - d->a[i];//Aé¢æ¶é´ååº
            else if(c->fa[i] != d->fa[i])
                return d->fa[i] - c->fa[i];//Aé¢+ç½æ¶éåº
        }
    }
        ...
        for(int i=0; i<r; i++)
        {
            ...
            if(ti<=300)
            {
                ...
                if(index==-1)
                    continue;
                id=ch-'A';
                if(team[index].pid[id]==1)//å·²ç»ååºæ¥äº
                    continue;
                else
                {
