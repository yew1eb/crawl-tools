...
void gauss(int n,int m)
{
    ...
    for(row=1,col=1;row<n,col<m;row++,col++)
    {
        ...
        for(i=row+1; i<=n; i++)  //ä¸»åä¸æ¯0æä¸é¢çè¡ç¬¬ä¸ä¸ªå¼å¨é¨åä¸º0
        {
            if(fabs(g[i][col])<eps)
                continue;
...
int main()
{
    ...
    while(cs--){
        ...
        memset(g,0,sizeof(g));
        int cnt=0;
        for(i=0;i<m;i++)  //i==mçæ¶ååªè½å¨å³è¾¹åºç°
            for(j=1;j<=n;j++)
            {
                cnt++;
                add(cnt,i,j,1.0);
