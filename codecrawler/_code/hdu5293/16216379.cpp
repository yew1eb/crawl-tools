...
void dfs2(int u,int f){
    ...
    for(int i=0;i<lis[u].size();i++){
        ...
        int tmp=Sum(l[s],ss)+Sum(l[t],ss)-Sum(l[s],sd)-Sum(l[t],sd)+sum[u];//Sum(l[s],ss)å³ä¸ºså°è¾¾rootçsumå½æ°å
        //ç±äºdfsçæ´æ°è¿ç¨æ¯ä»ä¸å¾ä¸çï¼æä»¥å¨æ´æ°uèç¹æ¶ï¼uåuçç¥åèç¹é½æ¯æ²¡ææ´æ°çï¼å æ­¤æ±åååªæ¯så°tè·¯å¾ä¸çæå¼å
        dp[u]=max(dp[u],tmp+w);
    }
