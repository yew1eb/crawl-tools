...
struct Node{
    int num,t;//å¯¹äºè®¢åæ¥è¯´numè¡¨ç¤ºæé¥¼æ°é,tè¡¨ç¤ºè®¢åæ¶é´;å¯¹äºå¶ä½æé¥¼æ¥è¯´numè¡¨ç¤ºèè´¹çä»·å¼,tè¡¨ç¤ºå¶ä½æ¶é´ 
}order[MAX],q[MAX*4];//qè¡¨ç¤ºåè°éå¢éå
...
int main(){
    ...
    while(~scanf("%d%d",&n,&m),n+m){
        for(int i=0;i<n;++i){
            ...
            order[i].num=r,order[i].t=Time(year,MON(mon),d,h);//è½¬åæå°æ¶ 
        }
