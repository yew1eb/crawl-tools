...
struct point
{
    ...
    double operator ^ (point p)//ç¹ç§¯
    {
        return x*p.x+y*p.y+z*p.z;
    }
    point operator * (point p)//åç§¯
    {
...
double a,b,c,d;
int n;
inline point get_point(point u,point v,point p)//på¨uvä¸çåè¶³
{
    double a,b,t;
...
point rotate(point u,point v,point p,double ang)//ç¹pæ²¿uvå³æangè§åº¦
{
...
void Graham(point* pol, int n, point* con, int& len){  //stä¸ºèµ·ç¹çä¸æ 
 sort(pol, pol+ n, cmp);
 con[0] = pol[0];
 con[1] = pol[1];
 int top = 1;
