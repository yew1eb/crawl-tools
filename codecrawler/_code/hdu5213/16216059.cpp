...
int main()
{
    while(~scanf("%d",&n)){
        ...
        for(int i=1;i<=m;i++)
        {
            ...
            /*åæ±(x,y)å¨l1~r2åºé´ææè§£æ°ï¼ç¶ååå»(x,y)å¨l1~l2-1,år1+1~r2åºé´è§£æ°ï¼
             å©ä½å³ä¸ºæ»¡è¶³xå¨l1~r1ä¸yå¨l2~r2åºé´çè§£æ°ï¼éå¤åçåå ä¸ãä»¥æ­¤è§£å³
             è«éç®æ³xï¼yå±äºä¸ååºé´çæ±è§£é®é¢*/
        }
        len=sqrt(n);//ååï¼lenä¸ºåé¿åº¦
        sort(f,f+tot,cmp);
        solve();
        for(int i=1;i<=m;i++)
        printf("%d\n",ans[i]);
