...
Created Time  :2014å¹´12æ03æ¥ ææä¸ 12æ¶28å41ç§
...
int judge(int i, int j) {
    ...
    int ok1 = son_judge(vec[i].up, vec[j]);//å¤æ­ç¹æ¯å¦å¨ç©å½¢åé¨
    int ok2 = son_judge(vec[i].down, vec[j]);
    int ok3 = son_judge(t1, vec[j]);
    int ok4 = son_judge(t2, vec[j]);
    if(ok1==1&&ok2==1&&ok3==1&&ok4==1)//ææåå­å½¢
        return 1;
    if(ok1==-1&&ok2==-1&&ok3==-1&&ok4==-1)//åä¸ªç¹å¨å¨å¶å¤é¨
        return -1;
    return 0;
}
