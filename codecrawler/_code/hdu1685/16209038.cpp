...
void dfs(int cnt)
{
    ...
    if(cnt+ceil(h()/3.0)>lim) return;//ceil(h()/3.0)è³å°è¿éè¦çæ­¥æ°ï¼å®éå¯è½æ¯ä»è¿å¤§

    int len,pos,j,p,temp[15];

    for(i=0;i<n;i++) temp[i]=num[i];//æåºåå­å°tempæ°ç»ä¸­
    ...
    for(i=0;i<n;i++) num[i]=temp[i];//å«å¿äºæ¢å¤æ°ç»
}
