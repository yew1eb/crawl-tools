...
void Init() //å¤çå¹³æ¹ãç«æ¹ãåæ¬¡æ¹
{
...
void Solve(const int &cur)
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (i == cur) continue;
        NUM &k = arr[i];
        if (k.num % curNum == 0) cnt[cur]++;    //factor
        if (curNum % k.num == 0) cnt[cur]++;    //multiple
        if (k.square == curNum) cnt[cur]++;     //other-square
        if (k.cube == curNum) cnt[cur]++;       //other-cube
        if (k.quad == curNum) cnt[cur]++;       //other-quad
        if (curNum % k.sum == 0) cnt[cur]++;        //other-sum
        if (k.multiple != 0 && curNum % k.multiple == 0) cnt[cur]++;            //other-multiple
    }
    if (square.count(curNum)) cnt[cur]++;       //square
    if (cube.count(curNum)) cnt[cur]++;         //cube
    if (quad.count(curNum)) cnt[cur]++;         //quad
    if (curNum % num.sum == 0) cnt[cur]++;      //sum-multiple
    if (num.multiple != 0 && curNum % num.multiple == 0) cnt[cur]++;    //multiple-multiple
    if (isPrime(curNum)) cnt[cur]++;    //prime
