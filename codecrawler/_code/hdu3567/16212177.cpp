...
int vis[2][maxn],t,Min_ans;//visè®°å½æç­å°åºç®æ çæ¶é´
char ans[maxn],get[maxn];
LL c[2][maxn];//c[i][j]è¡¨ç¤ºiç¶æä¸ï¼jåå¸å¼æ¶çæå°è·¯å¾
LL mm[30];  //mm[i]è¡¨ç¤º4^i
struct node{
    int f[3][3];
    int x,y;
    int g;
    int flag;//0è¡¨ç¤ºåBFSï¼1è¡¨ç¤ºåBFS
    LL path;
    int hash_num;
};
int get_hash(node e)//åº·æå±å¼ï¼åç¼©ç©ºé´ã
{
...
string get_str(LL c,int flag,int kk)//ä»æ°å­è½¬æ¢æè·¯å¾
{
    int str[100];
    int i,j,k=0;
...
void bfs(node e,node ee)//ååbfs
{
...
int main()
{
    //freopen("C:\\Documents and Settings\\All Users\\æ¡é¢\\in.txt","r",stdin);
    //freopen("C:\\Documents and Settings\\All Users\\æ¡é¢\\out1.txt","w",stdout);
    init();
    char a[30],b[30];
    int T,tt=0;
    scanf("%d",&T);
...
int main()
{
    freopen("C:\\Documents and Settings\\All Users\\æ¡é¢\\in.txt","w",stdout);
    srand(time(NULL));
    int i,j,k,n;
    printf("200\n");
    for(i=0;i<200;i++)
