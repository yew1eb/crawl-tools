...
inline point get_out_circle(point p[]){  //ä¸è§å½¢å¤æ¥ååå¿
...
inline void min_circle(int t,int ton){
        ...
        for(int i = 0; i < t; ++i){
                ...
                */
                ...
                /*æ¬é¢*/
                if(p[i].flag == 1){
                    if(dis(mc.c,p[i]) > mc.r + EPS){  //Aéåç¹å¨ååæåå¨ä¸
                        must_on[ton] = p[i];
                        min_circle(i,ton+1);
                    }
                }
                else {
                    if(dis(mc.c,p[i]) + EPS < mc.r){  //Béåçç¹ä¸å¨ååï¼å¯ä»¥å¨åå¨ä¸ï¼
                        must_on[ton] = p[i];
                        min_circle(i,ton+1);
                    }
                }
...
int main(){
        ...
        while (scanf("%d%d",&n,&m) != EOF){
                ...
                n = n + m;
                random_shuffle(p,p+n); //éæºååºååç´ 
                min_circle(n,0);
